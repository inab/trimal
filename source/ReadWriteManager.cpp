...
/* ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** */
/* This function is useful to detect the format from a given newAlignment */
/* ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** */
bool ReadWriteManager::loadAlignment(char *newAlignmentFile) {
    ...
    switch(iformat) {
        ...
        case 3:
            return loadNBRF_PirAlignment(newAlignmentFile);
        case 8:
            return loadFastaAlignment(newAlignmentFile);
        case 11:
            return loadPhylip3_2Alignment(newAlignmentFile);
        case 12:
            return loadPhylipAlignment(newAlignmentFile);
        case 17:
            return loadNexusAlignment(newAlignmentFile);
        case 21:
            return loadMegaInterleavedAlignment(newAlignmentFile);
        case 22:
            return loadMegaNonInterleavedAlignment(newAlignmentFile);
        default:
            return false;
            /* ***** ***** ***** ***** ***** ***** ***** ***** */
    }
...
/* ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** */
/* This function returns the input newAlignment format */
/* ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** */
...
/* ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** */
/* This function returns the input newAlignment type between a single or a
 *  multinewAlignment. */
/* ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** */
int ReadWriteManager::typeInputFile(void) {
...
/* ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** */
bool ReadWriteManager::printAlignment(void){
    ...
    if((_alignment->residNumber == 0)  || (_alignment->sequenNumber == 0)) {
        cerr << endl << "WARNING: Output newAlignment has not been generated. "
...
/* ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** */
/* This function puts the newAlignment in a given file */
/* ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** */
bool ReadWriteManager::saveAlignment(char *destFile) {
...
void ReadWriteManager::getSequences(ostream &file) {
    ...
    tmpMatrix = new string[_alignment->sequenNumber];
    ...
    for(i = 0; i < _alignment->sequenNumber; i++) {
        file << ">" << _alignment->seqsName[i] << endl;
...
int ReadWriteManager::formatInputAlignment(char *alignmentFile) {
    ...
    if((!strcmp(firstWord, "CLUSTAL")) || (!strcmp(firstWord, "clustal")))
        ...
        _alignment->sequenNumber = atoi(firstWord);
        firstWord = strtok(NULL, DELIMITERS);
        if(firstWord != NULL)
            _alignment->residNumber = atoi(firstWord);
        ...
        if((_alignment->sequenNumber == 1) && (_alignment->residNumber != 0))
            format = 12;

            /* If there are more than one sequence, analyze sequences distribution to
             * determine its format. */
        else if((_alignment->sequenNumber != 0) && (_alignment->residNumber != 0)) {
            blocks = 0;
...
bool ReadWriteManager::loadPhylipAlignment(char *alignmentFile) {
    ...
    return _alignment->fillMatrices(true);
}

bool ReadWriteManager::loadPhylip3_2Alignment(char *alignmentFile) {
...
bool ReadWriteManager::loadClustalAlignment(char *alignmentFile) {
...
bool ReadWriteManager::loadFastaAlignment(char *alignmentFile) {
...
bool ReadWriteManager::loadNexusAlignment(char *alignmentFile) {
...
bool ReadWriteManager::loadMegaNonInterleavedAlignment(char *alignmentFile) {
...
bool ReadWriteManager::loadMegaInterleavedAlignment(char *alignmentFile) {
...
bool ReadWriteManager::loadNBRF_PirAlignment(char *alignmentFile) {
...
void ReadWriteManager::alignmentPhylipToFile(ostream &file) {
    ...
    file << " " << _alignment->sequenNumber << " " << _alignment->residNumber << endl;

    /* First Block: Sequences Names & First 60 residues */
    for(i = 0; i < _alignment->sequenNumber; i++)
        file << setw(maxLongName + 3) << left << _alignment->seqsName[i].substr(0, maxLongName)
             << tmpMatrix[i].substr(0, 60) << endl;
    file << endl;

    /* Rest of blocks: Print 60 residues per each blocks of sequences */
    for(i = 60; i < _alignment->residNumber; i += 60) {
        for(j = 0; j < _alignment->sequenNumber; j++)
...
void ReadWriteManager::alignmentPhylip3_2ToFile(ostream &file) {
    ...
    for(i = 0; i < _alignment->sequenNumber; i++) {
        ...
        for(j = 0; j < _alignment->residNumber; j += 50) {
            for(k = j; (k < _alignment->residNumber) && (k < (j + 50)); k += 10)
                file << _alignment->sequences[i].substr(k, 10) << " ";
            file << endl;
...
void ReadWriteManager::alignmentPhylip_PamlToFile(ostream &file) {
...
void ReadWriteManager::alignmentClustalToFile(ostream &file) {
    ...
    for(j = 0; j < _alignment->residNumber; j += 60) {
        for(i = 0; i < _alignment->sequenNumber; i++)
            file << setw(maxLongName + 5) << left << _alignment->seqsName[i]
...
void ReadWriteManager::alignmentFastaToFile(ostream &file) {
...
void ReadWriteManager::alignmentNexusToFile(ostream &file) {
    ...
    file << "#NEXUS" << endl << "BEGIN DATA;" << endl << " DIMENSIONS NTAX="
         << _alignment->sequenNumber << " NCHAR=" << _alignment->residNumber <<";" << endl;
    ...
    else if (_alignment->dataType == AAType)
    ...
    for(i = 0; i < _alignment->sequenNumber; i++)
        file << "[Name: " << setw(maxLongName + 4) << left << _alignment->seqsName[i] << "Len: "
             << _alignment->residNumber << "]" << endl;
    file << endl << "MATRIX" << endl;

    /* Print alignment itself. Sequence name and 50 residues blocks */
    for(j = 0; j < _alignment->residNumber; j += 50) {
        for(i = 0; i < _alignment->sequenNumber; i++) {
            file << setw(maxLongName + 4) << left << _alignment->seqsName[i];
            for(k = j; k < (j + 50) && k < _alignment->residNumber; k += 10)
                file << " " << _alignment->sequences[i].substr(k, 10);
            file << endl;
        }
        file << endl;
...
void ReadWriteManager::alignmentMegaToFile(ostream &file) {
    ...
    file << "NSeqs=" << _alignment->sequenNumber << " Nsites=" << _alignment->residNumber
...
void ReadWriteManager::alignmentNBRF_PirToFile(ostream &file) {
    ...
    for(i = 0; i < _alignment->sequenNumber; i++) {
        ...
        if((_alignment->seqsInfo != NULL) && (iformat == oformat))
            ...
            file << ">" << alg_datatype << ";" << _alignment->seqsName[i] << endl
