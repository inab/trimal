...
void vcf_statish::readVCF(std::vector<newAlignment*> sources, std::vector<std::string> filenames, float minQuality, float minCoverage, bool ignoreFilter, const char * const replacementChar)
{
    ...
    applyVariantCallingFiles(sources , filenames, contigs, donorsPositions, minQuality, minCoverage, ignoreFilter, replacementChar);

    printApeek(sources);
}

void vcf_statish::printApeek(std::vector<newAlignment *> & sources)
{
...
void vcf_statish::extendAlignments(
...
void vcf_statish::applyVariantCallingFiles(
    std::vector<newAlignment*>      & sources , 
                            ...
                            for (std::string& s : donorsInfo)
                            {
                                if (s != "0")
                                {
                                            ...
                                            std::toupper(sources[i]->sequences[donorsPositions[C][counter] + 1][position - 1]) == std::toupper(ref[0]))
                                    {
                                        ...
                                        if (strlen(alt) > 1)
                                        {
                                            int curval = getTagFromCharArray(alt, ',');
                                            if (curval != -1)
                                            {
                                                sources[i]->sequences[donorsPositions[C][counter] + 1][position - 1] = getCharFromTag(curval);
                                            }
                                        }
                                        else
                                        {
...
void vcf_statish::obtainContigsAndDonors(
    std::vector<std::string> & filenames, 
    std::vector<std::string> & donors, 
    std::vector<std::string> & contigs, 
    std::vector<std::vector<int>> & donorsPositions)
...
int vcf_statish::getTagFromCharArray(char * array, char separator)
{
...
char vcf_statish::getCharFromTag(int tag)
{
    switch(tag)
    {
    ...
    // Two Bases
    case 3:  return 'M';// aMine AC
    case 5:  return 'W'; // Weak AT
    case 9:  return 'R'; // puRine AG
    case 6:  return 'Y';// pYrimidine CT
    case 10: return 'S';// Strong CG
    case 12: return 'K';// Keto TG

    // Three Bases
    case 14: return 'B';// CTG not A
    case 13: return 'D';// ATG not C
    case 11: return 'V';// ACG not T
    case 7:  return 'H';// ACT not G

    // All Four Bases
    default:
    case 15:return 'N';// ACTG
    }
}
