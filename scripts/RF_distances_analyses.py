...
def main():
  ...
  df = pd.read_csv("RF_distances_arrays_clean_Bacteria.csv")
  ...
  if args.plotType == "bar_chart":
    ...
    df_automated2 = df.loc[df["MSA_filter_tool"].str.contains("automated2"), :]
    ...
    plt.tight_layout()
    plt.savefig("{}.png".format("trimAl_gappyout-automated2"), bbox_inches='tight', dpi=150)
    

    for method in filter(lambda method_type: "automated2" in method_type, MSA_filter_tools_order):
      df_automated2_method = df.loc[df["MSA_filter_tool"] == method, :]
      ...
      plt.tight_layout()
      plt.savefig("{}.png".format("trimAl_gappyout_" + method), bbox_inches='tight', dpi=150)
      #plt.show()
